...
test.describe("Attendance API", () => {
  ...
  test("GET /attendance/today returns status with valid auth", async ({
    request,
  }) => {
    await ensureAuth(request);
    test.skip(!accessToken, "Login not available");
    const response = await request.get("./attendance/today", {
  ...
  test("GET /attendance/today response has correct envelope", async ({
    request,
  }) => {
    await ensureAuth(request);
    test.skip(!accessToken, "Login not available");
  ...
  test("GET /attendance/realtime returns realtime attendance", async ({
    request,
  }) => {
    await ensureAuth(request);
    test.skip(!accessToken, "Login not available");
    ...
    const response = await withRateLimitRetry(() =>
      request.get("./attendance/realtime", {
        headers: { Authorization: `Bearer ${accessToken}` },
      }),
    );
  ...
  test("GET /attendance/site/:siteId/report returns site report", async ({
    request,
  }) => {
    await ensureAuth(request);
    test.skip(!accessToken, "Login not available");
    ...
    const response = await withRateLimitRetry(() =>
      request.get("./attendance/site/test-site/report", {
        headers: { Authorization: `Bearer ${accessToken}` },
      }),
    );
    const status = response.status();
  ...
  test.describe("Attendance Admin Endpoints", () => {
    test("POST /attendance/sync syncs attendance events with admin auth", async ({
      request,
    }) => {
      ...
      const response = await withRateLimitRetry(() =>
        request.post("./attendance/sync", {
          headers: {
            Authorization: `Bearer ${adminAccessToken}`,
            "Idempotency-Key": `e2e-attendance-sync-${Date.now()}`,
          },
          data: { events: [] },
        }),
      );
